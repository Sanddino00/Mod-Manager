...
class ModManager(QWidget):
    ...
    def init_ui(self):
        ...
        # Top layout: Game selection + update dot
        ...
        self.update_dot.setFont(QFont("Arial", 16, QFont.Weight.Bold))
        self.update_dot.setStyleSheet("color: red;")
    ...
    def create_settings_tab(self):
        ...
        # Version label
        self.version_label = QLabel(f"Version: {settings.get('version','1.0.7')}")
    ...
    # -------------------- UPDATE FUNCTIONS --------------------
    def check_for_update_dot(self):
        try:
            r = requests.get(GITHUB_RELEASES, allow_redirects=True)
            latest_tag = r.url.split('/')[-1].replace('v','')
            current_version = settings.get("version","1.0.6")
            if tuple(map(int,latest_tag.split('.'))) > tuple(map(int,current_version.split('.'))):
                self.update_dot.setStyleSheet("color: green;")
            else:
                self.update_dot.setStyleSheet("color: red;")
        except:
            self.update_dot.setStyleSheet("color: red;")
    ...
    def update_installer(self):
        ...
        if os.path.exists(update_new_path):
            old_path = os.path.join(BASE_DIR,"update.exe")
            if os.path.exists(old_path):
                os.remove(old_path)
            os.rename(update_new_path, old_path)
        else:
            url = "https://github.com/Sanddino00/Mod-Manager/releases/latest/download/update.exe"
            r = requests.get(url, allow_redirects=True)
