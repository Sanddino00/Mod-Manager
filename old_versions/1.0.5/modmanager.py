...
import sys, os, json, shutil, subprocess, requests, zipfile
from PyQt6.QtWidgets import (
    QApplication, QWidget, QLabel, QVBoxLayout, QHBoxLayout, QPushButton,
    QComboBox, QTabWidget, QGridLayout, QScrollArea, QFrame, QFileDialog, QListWidget, QListWidgetItem, QProgressBar
)
...
SCRIPT_VERSION = "1.0.5"  # read this in settings
...
if not os.path.exists(SETTINGS_FILE):
    ...
    settings["version"] = SCRIPT_VERSION  # Ensure settings version matches script
...
class ModManager(QWidget):
    ...
    def init_ui(self):
        ...
        top_layout.addWidget(QLabel("Update Available"))
        ...
        self.apply_theme()
        self.check_update_availability()
    ...
    def create_settings_tab(self):
        ...
        # Move theme toggle, version and update buttons a bit down
        ...
        self.version_label = QLabel(f"Version: {settings['version']}")
        self.version_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.settings_layout.addWidget(self.version_label)

        # Update buttons
        self.update_modmanager_btn = QPushButton("Update ModManager")
        self.update_modmanager_btn.clicked.connect(self.update_modmanager)
        self.settings_layout.addWidget(self.update_modmanager_btn)

        self.update_updater_btn = QPushButton("Update Installer / update.exe")
        self.update_updater_btn.clicked.connect(self.update_updater)
        self.settings_layout.addWidget(self.update_updater_btn)
    ...
    def check_update_availability(self):
        # Placeholder logic for demo: always red dot
        self.update_dot.setStyleSheet("color: red; font-weight: bold;")

    def update_modmanager(self):
        # Launch the updater, it handles downloading the latest ModManager and resources
    ...
    def update_updater(self):
        # Download latest update.exe if update_new.exe not present
        new_updater_path = os.path.join(BASE_DIR, "update_new.exe")
        if os.path.exists(new_updater_path):
            print("Using existing update_new.exe")
            return
        # Download latest release from GitHub
        url = "https://github.com/Sanddino00/Mod-Manager/releases/latest/download/update.exe"
        try:
            r = requests.get(url, stream=True)
            with open(new_updater_path, "wb") as f:
