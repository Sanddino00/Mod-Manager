...
GITHUB_RELEASE_URL = "https://github.com/Sanddino00/Mod-Manager/releases/latest/download"
...
class ModManager(QWidget):
    ...
    def init_ui(self):
        ...
        # Top: Game selection + update status
    ...
    def create_settings_tab(self):
        ...
        self.version_label = QLabel(f"Version: {settings.get('version','1.0.6')}")
        self.version_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.settings_layout.addWidget(self.version_label)

        # -------------------- Update buttons --------------------
        self.update_modmanager_btn = QPushButton("Update Mod-Manager")
        ...
        self.update_installer_btn = QPushButton("Update Installer")
        self.update_installer_btn.clicked.connect(self.update_installer)
        self.settings_layout.addWidget(self.update_installer_btn)

        self.settings_layout.addStretch()

    # -------------------- UPDATE DOT --------------------
    def check_update_available(self):
        # You could implement a version check here if needed
        # For demo: always red
        self.update_dot.setStyleSheet("color: red; font-weight: bold;")
        self.update_label.setText("Update available")

    # -------------------- UPDATE FUNCTIONS --------------------
    def update_modmanager(self):
        # Launch update.exe to handle modmanager & resources updates
    ...
    def update_installer(self):
        # Use update_new.exe if exists
        update_new_path = os.path.join(BASE_DIR, "update_new.exe")
        update_path = os.path.join(BASE_DIR, "update.exe")
        if not os.path.exists(update_new_path):
            # Download update_new.exe
            try:
                url = f"{GITHUB_RELEASE_URL}/update.exe"
                r = requests.get(url, stream=True)
                if r.status_code == 200:
                    with open(update_new_path, "wb") as f:
                        shutil.copyfileobj(r.raw, f)
            except Exception as e:
                print(f"Failed to download update.exe: {e}")
                return

        # Delete old update.exe if exists
        if os.path.exists(update_path):
            try:
                os.remove(update_path)
            except Exception as e:
                print(f"Failed to delete old update.exe: {e}")

        # Rename downloaded update_new.exe -> update.exe
        try:
            os.rename(update_new_path, update_path)
        except Exception as e:
            print(f"Failed to rename update_new.exe: {e}")
