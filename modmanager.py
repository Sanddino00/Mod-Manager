import sys, os, json, shutil, subprocess, zipfile, requests, time
...
class ModManager(QWidget):
    ...
    def init_ui(self):
        ...
        self.update_dot.setFixedSize(12,12)
        self.update_dot.setStyleSheet("background-color: red; border-radius: 6px;")
    ...
    def check_for_update(self):
        try:
            ...
            if self.version_tuple(latest_tag) > self.version_tuple(self.version):
                self.update_dot.setStyleSheet("background-color: green; border-radius: 6px;")
                self.update_label.setText(f"Update Available ({latest_tag})")
            else:
                self.update_dot.setStyleSheet("background-color: red; border-radius: 6px;")
                self.update_label.setText("No Update")
        except Exception as e:
    ...
    def update_exe(self):
        ...
        exe_url = "https://github.com/Sanddino00/Mod-Manager/releases/latest/download/modmanager.exe"
        new_exe_path = os.path.join(base_dir, "modmanager_new.exe")
        ...
        try:
            ...
            print(f"Failed to download EXE: {e}")
            return
        subprocess.Popen([updater_path], cwd=base_dir)
        QApplication.quit()
        sys.exit()

    def update_resources(self):
        base_dir = os.path.dirname(os.path.abspath(sys.argv[0]))
        zip_url = "https://github.com/Sanddino00/Mod-Manager/releases/latest/download/resources.zip"
        ...
        try:
            ...
            print(f"Failed to download resources.zip: {e}")
            return
        try:
            ...
            print(f"Failed to extract resources.zip: {e}")
